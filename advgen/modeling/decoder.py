...
import advgen.structs
import advgen.utils_cython
from advgen.modeling.lib import PointSubGraph, GlobalGraphRes, CrossAttention, GlobalGraph, MLP

import advgen.utils, advgen.globals
...
class Decoder(nn.Module):
    ...
    def __init__(self, args_: advgen.utils.Args, vectornet):
    ...
    def goals_2D_per_example(self, i: int, goals_2D: np.ndarray, mapping: List[Dict], lane_states_batch: List[Tensor],
        ...
        if 'tnt_square' in args.other_params:
            ...
            mapping[i]['tnt_square.pack'] = [
                *advgen.utils_cython.tnt_square_filter(goals_2D, advgen.utils.to_numpy(scores), args),
                gt_goal,
            ]

            if args.do_eval:
                if 'single2joint' in args.other_params:
                    advgen.utils.select_goals_by_NMS(mapping[i], goals_2D, np.array(scores.tolist()), args.nms_threshold, mapping[i]['speed'])

        else:
            if args.do_eval:
                if args.nms_threshold is not None:
                    advgen.utils.select_goals_by_NMS(mapping[i], goals_2D, np.array(scores.tolist()), args.nms_threshold, mapping[i]['speed'],mode_num=args.mode_num)
                else:
                    pass
    ...
    def goals_2D_eval(self, batch_size, mapping, labels, hidden_states, inputs, inputs_lengths, device):
        ...
        if args.visualize:
            for i in range(batch_size):
                advgen.utils.visualize_goals_2D(mapping[i], mapping[i]['vis.goals_2D'], mapping[i]['vis.scores'], self.future_frame_num,
                                         labels=mapping[i]['vis.labels'],
                                         labels_is_valid=mapping[i]['vis.labels_is_valid'],
                                         predict=mapping[i]['vis.predict_trajs'])

        if 'pred_yaw' in args.other_params:
            return pred_trajs_batch, pred_probs_batch, pred_yaws_batch, pred_indices_batch
        else:
            return pred_trajs_batch, pred_probs_batch, pred_indices_batch
    ...
    def variety_loss(self, mapping: List[Dict], hidden_states: Tensor, batch_size, inputs: Tensor,
        ...
        for i in range(batch_size):
            ...
            argmin = np.argmin(advgen.utils.get_dis_point_2_points(gt_points[-1], np.array(outputs[i, :, -1, :].tolist())))
            ...
            if labels_is_valid[i].sum() > advgen.utils.eps:
        ...
        if args.do_eval:
            ...
            if args.waymo:
                for i in range(batch_size):
                    normalizer: advgen.utils.Normalizer = mapping[i]['normalizer']
                ...
                for i in range(batch_size):
                    for each in outputs[i]:
                        advgen.utils.to_origin_coordinate(each, i)
    ...
    def forward(self, mapping: List[Dict], batch_size, lane_states_batch: List[Tensor], inputs: Tensor,
        ...
        if 'direct_relation_label' in args.other_params:
            ...
            interaction_labels = advgen.utils.get_from_mapping(mapping, 'influencer_idx')
            ...
            for i in range(batch_size):
                ...
                for j, label in enumerate(interaction_labels[i]):
                    ...
                    if label == 1:
                        advgen.utils.other_errors_put('directR_inf_1', ok)
                    elif label == 0:
                        advgen.utils.other_errors_put('directR_inf_0', float(ok))
                    else:
        ...
        if 'train_relation' in args.other_params:
            ...
            interaction_labels = advgen.utils.get_from_mapping(mapping, 'interaction_label')
            loss += F.nll_loss(outputs, torch.tensor(interaction_labels, dtype=torch.long, device=device))
            argmax = torch.argmax(outputs, dim=-1)
            for i in range(batch_size):
                if 'pred_with_threshold' in args.other_params and args.do_eval:
                ...
                advgen.utils.other_errors_put(f'interaction_label.{interaction_labels[i]}', float(ok))
                advgen.utils.other_errors_put('interaction_label.all', float(ok))
                # this blows gpu memory when testing
        ...
        if 'variety_loss' in args.other_params:
            ...
            if 'tnt_square' in args.other_params:
                for i in range(0, batch_size, 2):
                    ...
                    gt_goal_idx = np.argmin(advgen.utils.get_dis_point_2_points(gt_goal, goals_2D))
                    gt_goal_idx_oppo = np.argmin(advgen.utils.get_dis_point_2_points(gt_goal_oppo, goals_2D_oppo))
                    ...
                    goals_2D_norm = advgen.utils_cython.get_normalized_points(
                        advgen.utils_cython.get_normalized_points(goals_2D, normalizer, reverse=True),
                        normalizer_oppo
                    )
                    goals_2D_oppo_norm = advgen.utils_cython.get_normalized_points(
                        advgen.utils_cython.get_normalized_points(goals_2D_oppo, normalizer_oppo, reverse=True),
                        normalizer
                    )
                    ...
                    if args.do_eval:
                        ...
                        advgen.utils.select_goal_pairs_by_NMS(mapping[i], mapping[i + 1], goals_4D_raw, advgen.utils.to_numpy(joint_scores),
                                                       args.nms_threshold,
                                                       mapping[i]['speed'],
                                                       mapping[i + 1]['speed'],
                                                       args)
            ...
            if args.do_eval:
                ...
                if args.visualize:
                    for i in range(batch_size):
                        ...
                        advgen.utils.visualize_goals_2D(mapping[i], mapping[i]['vis.goals_2D'], mapping[i]['vis.scores'],
