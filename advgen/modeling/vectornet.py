...
from advgen.modeling.decoder import Decoder, DecoderResCat
from advgen.modeling.lib import MLP, GlobalGraph, LayerNorm, SubGraph, CrossAttention, GlobalGraphRes
import advgen.utils
...
class VectorNet(nn.Module):
    ...
    def __init__(self, args_: advgen.utils.Args):
    ...
    def forward_encode_sub_graph(self, mapping: List[Dict], matrix: List[np.ndarray], polyline_spans: List[slice],
        ...
        """
        if 'raster' in args.other_params:
            raster_images = advgen.utils.get_from_mapping(mapping, 'image')
        ...
        element_states_batch = advgen.utils.merge_tensors_not_add_dim(input_list_list, module=self.sub_graph,
                                                               sub_batch_size=16, device=device)

        if 'laneGCN' in args.other_params:
            inputs_before_laneGCN, inputs_lengths_before_laneGCN = advgen.utils.merge_tensors(element_states_batch, device=device)
            for i in range(batch_size):
                map_start_polyline_idx = mapping[i]['map_start_polyline_idx']
    ...
    def forward(self, mapping: List[Dict], device, return_tensors_for_dpo=False):
        ...
        matrix = advgen.utils.get_from_mapping(mapping, 'matrix')
        # vectors of i_th element is matrix[polyline_spans[i]]
        polyline_spans = advgen.utils.get_from_mapping(mapping, 'polyline_spans')
