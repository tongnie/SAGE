...
import advgen.globals, advgen.utils_cython, advgen.structs
...
def select_goals_by_NMS(mapping: Dict, goals_2D: np.ndarray, scores: np.ndarray, threshold, speed, gt_goal=None, mode_num=6):
    ...
    speed_scale_factor = advgen.utils_cython.speed_scale_factor(speed)
...
def select_goal_pairs_by_NMS(mapping: Dict, mapping_oppo: Dict, goals_4D: np.ndarray, scores_4D: np.ndarray, threshold, speed, speed_oppo, args,
    ...
    thresholds = (threshold * advgen.utils_cython.speed_scale_factor(speed), threshold * advgen.utils_cython.speed_scale_factor(speed_oppo))
...
def get_static_var(obj, name, default=None, path=None):
    if not hasattr(obj, name):
        if default is not None:
            ...
            value = advgen.structs.load(path)
        else:
            assert False
