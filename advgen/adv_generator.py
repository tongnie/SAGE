...
class AdvGenerator():
    ...
    def after_episode(self,update_AV_traj=False,mode='train',logging=None,n=10):
        if update_AV_traj:
            latest_ego_traj = np.array(self.ego_traj)[11:91]
            if len(latest_ego_traj)<n:
                if logging == None:
                    print('Ignore traj less than 1s')
                else:
                    logging.info('Ignore traj less than 1s')
                return
            else:
                if logging == None:
                    print('Latest traj is updated')
                else:
                    logging.info('Latest traj is updated')
            if mode == 'train':
    ...
    def generate(self,mode='train'):
        ...
        if mode == 'train':
            trajs_list = self.storage[self.env.current_seed].get('AV_trajs')
            trajs_AV = [np.array(traj) for traj in trajs_list]
            # trajs_AV = np.array(self.storage[self.env.current_seed].get('AV_trajs'))
            probs_AV = np.array(self.storage[self.env.current_seed].get('AV_probs'))
        elif mode == 'eval':
            trajs_list = self.storage[self.env.current_seed].get('AV_trajs_eval')
            trajs_AV = [np.array(traj) for traj in trajs_list]
