...
class ScenarioTrafficManager(BaseManager):
        ...
        if self.episode_step < self.current_scenario_length:
            ...
            for scenario_id, track in self.current_traffic_data.items():
                if scenario_id not in self._scenario_id_to_obj_id:
                    ...
                    if vname in self.adv_name and len(self.adv_traj[self.adv_name.index(vname)]):
                        sample = self.adv_traj[self.adv_name.index(vname)].pop(0)
                        v = self.v_map[vname]
    ...
    def set_adv_info(self,adv_name,adv_traj):
        self.adv_name.append(adv_name)
        self.adv_traj.append(adv_traj)
