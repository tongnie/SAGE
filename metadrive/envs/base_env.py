...
class BaseEnv(gym.Env):
    ...
    def __init__(self, config: dict = None):
        ...
        if self.config['logger'] == None:   # 20250729 modify the logger
            for h in logging.root.handlers[:]:
                logging.root.removeHandler(h)
            self.logger = logging.getLogger(self.config['logs_path'])
            self.logger.propagate = False
            formatter = logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s',
                                  datefmt='%Y-%m-%d %H:%M:%S')
            file_handler = logging.FileHandler(self.config['logs_path'], mode='a') # 'a' for append, 'w' for overwrite
